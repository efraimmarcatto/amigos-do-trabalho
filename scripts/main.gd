...
# Container node for all furniture (z-ordered behind pet) — references Furnitures node in scene tree
@onready var _furniture_container: Node2D = $Furnitures
...
func _ready() -> void:
	...
	# No z_index needed — tree order handles rendering: Furnitures → PetSprite → UI Controls
